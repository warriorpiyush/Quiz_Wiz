...
function DocumentUpload({ setMethod }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const extractedText = await extractTextPdf(file);

      if (!extractedText || extractedText.length < 100) {
        setError("The document doesn't contain enough text to generate questions. Please try a different document.");
        setLoading(false);
        return;
      }
      ...
      const response = await generateQuestions({
        num_questions: "5",
        text: extractedText,
        prompt: "Generate quiz questions based on this document content:",
      });
    ...
    <div className="bg-gray-100 flex flex-col min-h-screen">
      ...
      <div className="flex flex-col justify-center items-center p-8">
        <h1 className="text-3xl font-rubik font-bold mb-4 flex border-b-4 w-2/3 items-center justify-center border-purple-300 rounded">
          ...
          <img src={FILE_SVG} className="w-20 p-3" alt="Document icon" />
          <span>Documents</span>
        </h1>
