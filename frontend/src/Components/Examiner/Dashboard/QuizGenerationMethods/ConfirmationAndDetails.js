...
function ConfirmationAndDetails({ setMethod }) {
  const { user: localUser, questions, setQuestions } = useAppContext();
  ...
  const [formData, setFormData] = useState({
    ...
    generator: user?.email || "unknown@example.com", // Will be updated when user is available
  });

  // Update generator field when user data becomes available
  useEffect(() => {
    if (user?.email) {
      setFormData(prevData => ({
        ...prevData,
  ...
  const handleCreateQuiz = async () => {
    console.log('🎯 Creating quiz with user data:', {
      localUser: localUser,
    ...
    if (!user?.email) {
      setSnackbarMessage("User email not found. Please log in again.");
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
      return;
    }
    ...
    try {
      console.log('📝 Quiz data being sent:', formData);
